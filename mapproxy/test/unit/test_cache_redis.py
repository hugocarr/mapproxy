...
class TestRedisCache(TileCacheTestBase):
    ...
    def setup_method(self):
        ...
        if os.environ.get('MAPPROXY_TEST_REDIS_CLUSTER'):
            redis_cluster = os.environ['MAPPROXY_TEST_REDIS_CLUSTER'].split(",")
            self.cluster_host_port = [
                (host, port) for host, port in (host_port.split(':') for host_port in redis_cluster)
            ]
    ...
    def test_cluster(self):
        for host, port in self.cluster_host_port:
            cache = RedisClusterCache(host, port, prefix='mapproxy-test', db=1)
